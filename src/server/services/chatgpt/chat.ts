...
const createChatCompletionStreaming = async (
  messages: ChatCompletionRequestMessage[]
): Promise<any> => {
  return await openaiService.createChatCompletion(
    {
...
const createChatCompletionWithModelStreaming = async (
  messages: string,
  modelId: string
) => {
  return await openaiService.createCompletion(
    {
      model: modelId,
      prompt: messages,
      stream: true,
...
const createWhisperTranscription = async (formData: FormData) => {
  ...
  return await axios.post<CreateTranscriptionResponse>(
    openaiHost + 'audio/transcriptions',
    formData,
    {
      headers: {
...
const convertResponseToRequestMessage = (
  respMessages: CreateChatCompletionResponseChoicesInner[]
) => {
...
export {
  ...
  createChatCompletionWithModelStreaming,
